...
namespace VoiceTexterBot
{
    class Bot : BackgroundService
    {
        ...
        async Task HandleUpdateAsync(ITelegramBotClient botClient, Update update, CancellationToken cancellationToken)
        {
            ...
            //обрабатываем входящие сообщения из Telegram Bot API: http://core.telegram.org/bots/api#message
            if (update.Type == UpdateType.Message)
            {
                Console.WriteLine($"Получено сообщение {update.Message.Text}");
                await
                    _telegramClient.SendTextMessageAsync(update.Message.Chat.Id, $"Вы отправили сообщение {update.Message.Text}", cancellationToken: cancellationToken);
                return;
            }
        }
